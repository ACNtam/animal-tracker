...
const Sightings = () => {
    ...
    const handleAddSighting = async (e) => {
        ...
        const response = await fetch('http://localhost:8081/sightings', {
            method: 'POST',
            headers: {
    ...
    const handleDeleteSighting = async (deleteId) => {
        const response = await fetch(`http://localhost:8081/sightings/${deleteId}`, {
            method: 'DELETE',
        })
        await response.json();
